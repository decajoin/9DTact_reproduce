...
class Camera:
    def __init__(self, cfg, calibrated=True):
        ...
        # (自加)为了不污染原始原件，增加测试标定数据存储位置
        self.calibration_test_dir = calibration_root_dir + '/test_' + str(sensor_id)
        ...
        # 同上
        self.camera_test_calibration_dir = self.calibration_test_dir + camera_calibration['camera_calibration_dir']
        # 同上
        ...
        self.row_test_index_path = self.camera_test_calibration_dir + camera_calibration['row_index_path']
        self.col_test_index_path = self.camera_test_calibration_dir + camera_calibration['col_index_path']
        self.position_test_scale_path = self.camera_test_calibration_dir + camera_calibration['position_scale_path']
        ## 裁切后的图像大小，即标定板凸点区域的像素位置
    ...
    def get_raw_avg_image(self):
        ...
        global img
        while True:
            img = self.cap.read()[1] # [0]:是否成功读取到帧，布尔值；[1]:读取到的帧，三通道
            cv2.imshow('img', img)
            key = cv2.waitKey(1)
            if key == ord('y'):
